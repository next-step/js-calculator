...
describe('계산기 앱', () => {
	...
	describe('2개의 숫자에 대해 덧셈이 가능하다.', () => {
		...
		it('0+1=1, 이때 첫 값은 0이므로 0을 입력하지 않는다.', () => {
			cy.findByRole('button', {name: /\+/i}).click();
			cy.findByRole('button', {name: /1/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
	...
	describe('2개의 숫자에 대해 뺄셈이 가능하다.', () => {
		...
		it('0-2=-2 이떄 첫 값은 0이므로 0을 입력하지 않는다.', () => {
			cy.findByRole('button', {name: /\-/i}).click();
			cy.findByRole('button', {name: /2/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
			cy.findByRole('heading', {level: 1}).should('have.text', '-2');
	...
	describe('2개의 숫자에 대해 곱셈이 가능하다.', () => {
		...
		it('0*2=0 이때 첫 값은 0이므로 0을 입력하지 않는다.', () => {
			cy.findByRole('button', {name: /x/i}).click();
			cy.findByRole('button', {name: /2/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
			cy.findByRole('heading', {level: 1}).should('have.text', '0');
		});
		it('1*0=0 이때 첫 값은 0이므로 0을 입력하지 않는다.', () => {
			cy.findByRole('button', {name: /x/i}).click();
	...
	describe('2개의 숫자에 대해 나눗셈이 가능하다.', () => {
		...
		it('1/0=Infinity', () => {
			...
			cy.findByRole('button', {name: /0/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
			cy.findByRole('heading', {level: 1}).should('have.text', 'Infinity');
		});
		it('0/0=NaN 이때 첫 값은 0이므로 0을 입력하지 않는다.', () => {
			cy.findByRole('button', {name: /\//i}).click();
			cy.findByRole('button', {name: /0/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
			cy.findByRole('heading', {level: 1}).should('have.text', 'NaN');
		});
		it('0/0=NaN 첫값0을 입력해본다.', () => {
			cy.findByRole('button', {name: /0/i}).click();
			cy.findByRole('button', {name: /\//i}).click();
			cy.findByRole('button', {name: /0/i}).click();
			cy.findByRole('button', {name: /\=/i}).click();
