import { EXCEPT_EQUAL_OPERATORS_ARRAY, LENGTH_LIMIT_TEXT, MAX_NUMBER_LENGTH } from './constants.js';
import checkNotNumber from './regex.js';

const handleNumber = (number, input) => {
  const result = input.concat(number);
  const numberLengthList = result.split(checkNotNumber).map((num) => num.length);
  const findOperatorText = String(result.match(checkNotNumber));
  const noAlertCondition = EXCEPT_EQUAL_OPERATORS_ARRAY.includes(findOperatorText);
  ...
  for (const [index, numberLength] of numberLengthList.entries()) {
    ...
    if (!(index === 0 && noAlertCondition) && numberLength > MAX_NUMBER_LENGTH) {
